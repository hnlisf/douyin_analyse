...
class VideoParser:
    ...
    def  __init__(self):
        ...
        # 视频点赞数
        self._digg_count = 0
        # TODO:老视频无此参数，怀疑是视频提升次数，抖音推荐算法中，根据用户视频点赞数和评论数的百分比，不断提升到更大的用户池
    ...
    with open("../douyin_data/douyin_video88.json","r") as f:
        for index,line in enumerate(f):
            if index >500000:
                ...
                print("##################################################")
                print("index:",index)
                print("uid:",video_parser.uid,type(video_parser.uid))
                print("aweme_id:",video_parser.aweme_id,type(video_parser.aweme_id))
                print("title:",video_parser.title,type(video_parser.title))
                print("play_url_list:",video_parser.play_url_list,type(video_parser.play_url_list))
                print("cover_url_list:",video_parser.cover_url_list,type(video_parser.cover_url_list))
                print("digg_count:",video_parser.digg_count,type(video_parser.digg_count))
                print("forward_count:",video_parser.forward_count,type(video_parser.forward_count))
                print("create_time:",video_parser.create_time,type(video_parser.create_time))
                print("rating:",video_parser.rating,type(video_parser.rating))
